...
export default function PurchaseEntry() {
  ...
  async function load() {
    ...
    if (farmerList && farmerList.length > 0) {
      const branchFarmers = farmerList.filter((f: any) => f.branch_id === prof.branch_id);
      setFarmerId((prev) => prev || branchFarmers[0]?.id || "");
    }
  ...
  const farmersForBranch = farmers.filter((f) => f.branch_id === branchId);
  ...
  const pct = gross > 0 ? Math.round((discountValue / gross) * 10000) / 100 : 0;
  ...
  return (
    <>
      ...
      <main className="min-h-screen p-8 max-w-lg mx-auto">
        <p className="no-print font-mono text-xs tracking-widest text-gold uppercase mb-1">Agrobuyer</p>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const firstMatch = farmers.find((f) => f.branch_id === e.target.value);
                setFarmerId(firstMatch ? firstMatch.id : "");
              }}
            >
              {branches.map((b) => (
        ...
        <div>
          <label className="text-xs opacity-60 block mb-1">Quality discount (NGN)</label>
          <input
            type="number"
            className="w-full bg-surface border border-white/10 rounded-md p-3"
        ...
        <div className="border border-white/10 rounded-lg p-4 space-y-2 text-sm">
          ...
          <div className="flex justify-between"><span className="opacity-60">Quality discount</span><span className="font-mono text-rust">- NGN {discountValue.toLocaleString()}</span></div>
